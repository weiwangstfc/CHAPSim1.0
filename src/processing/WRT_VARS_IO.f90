...
!**********************************************************************************************************************************
SUBROUTINE WRT_INSTANT_VARS_IO
    use cparam, only : iThermoDynamics

    CALL WRT_INSTANT_VARS_FLOW_IO
    IF(iThermoDynamics == 1) CALL WRT_INSTANT_VARS_THERMAL_IO

END SUBROUTINE WRT_INSTANT_VARS_IO

SUBROUTINE WRT_INSTANT_VARS_FLOW_IO
    USE MESH_INFO
    USE FLOW_INFO
    USE INIT_INFO
    USE WRT_INFO
    ...
    ALLOCATE(DUMMY(1 : NCL1_IO, 1 : N2DO(MYID), 1 : NCL3))
    DUMMY = 0.0_WP
    ...
    DO IVEL = 1, NDV + 1
        ...
        DO I = 1, NCL1_IO
            DO J = 1, N2DO(MYID)
                DO K = 1, NCL3
                    IF(IVEL == 1) DUMMY(I, J, K) = Q_IO(I, J, K, 1) !U
                    IF(IVEL == 2) DUMMY(I, J, K) = Q_IO(I, J, K, 2) !V
                    IF(IVEL == 3) DUMMY(I, J, K) = Q_IO(I, J, K, 3) !W
...
SUBROUTINE WRT_INSTANT_VARS_THERMAL_IO
    USE MESH_INFO
    USE FLOW_INFO
    USE thermal_info
    USE INIT_INFO
