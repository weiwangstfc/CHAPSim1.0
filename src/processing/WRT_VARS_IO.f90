...
SUBROUTINE WRT_INSTANT_VARS_io
    ...
    DO IVEL = 1, NDV + 1
        ...
        ALLOCATE(DUMMY(NCL1S : NCL1E, 1 : N2DO(MYID), 1 : NCL3))
        DUMMY(:, :, :) = 0.0_WP
        !==================GENERATE FILE NAMES ===============================================
        DFLG(IVEL) = 100 + IVEL
        ...
        DO I = NCL1S, NCL1E
            DO J = 1, N2DO(MYID)
                DO K = 1, NCL3
                    ...
                    IF(IVEL == 4) DUMMY(I, J, K) = PR_io(I, J, K)
                    !print*, DUMMY(I, J, K)
                END DO
            END DO
        END DO

        SIZES_ARRAY(1) = NCL1E - NCL1S + 1
        SIZES_ARRAY(2) = NCL2
        SIZES_ARRAY(3) = NCL3
        ...
        CALL MPI_FILE_OPEN(MPI_COMM_WORLD, TRIM(WRT_RST_FNM_io(IVEL)), MPI_MODE_WRONLY + MPI_MODE_CREATE, &
            MPI_INFO_NULL, MYFILE, IERR)

        INTMPI(1) = NCL1E - NCL1S + 1
        INTMPI(2) = NCL2
        INTMPI(3) = NCL3
        INTMPI(4) = ITERG
        ...
        CALL MPI_FILE_SET_VIEW(MYFILE,OFFSET, MPI_DOUBLE_PRECISION, MPI_DOUBLE_PRECISION, &
    ...
    IF(iThermoDynamics == 1) THEN
        DO IVEL = 1, 5
            ...
            MPI_DOUBLE_PRECISION, NEWTYPE, IERR)
            CALL MPI_TYPE_COMMIT(NEWTYPE, IERR)
            CALL MPI_FILE_OPEN(MPI_COMM_WORLD,WRT_RST_FNM_io(IVEL), MPI_MODE_WRONLY + MPI_MODE_CREATE, MPI_INFO_NULL, MYFILE, IERR)
...
SUBROUTINE WRT_AVERAGE_VARS_nonXperiodic_io
    ...
    CALL MPI_TYPE_COMMIT(NEWTYPE, IERR)
    CALL MPI_FILE_OPEN(MPI_COMM_WORLD,WRT_AVE_FNM_io, MPI_MODE_WRONLY + MPI_MODE_CREATE, MPI_INFO_NULL, MYFILE, IERR)
